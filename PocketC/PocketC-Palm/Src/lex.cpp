...
start_over:
    ...
    if (isdigit(ch())) { // || ((ch()=='-' || ch()=='+') && isdigit(nch()))) {
        ...
        */
        ...
        while (ch() && (isdigit(ch()) || ch()=='.' || lower(ch()) == 'e' || (isHex && lower(ch()) >='a' && lower(ch()) <='f'))) {
            if (ch() == '.' || (!isHex && lower(ch())=='e')) isFloat = true;
            if (lower(ch()) == 'e' && (nch() == '-' || nch() == '+') && !isHex) val[istr++] = gch();
            val[istr++] = gch();
