...
void CCrystalTextBuffer::SetSourceType(const TCHAR* pszFileName)
{
    char ext[10];
    _tsplitpath(pszFileName, NULL, NULL, NULL, ext);

    m_srcType = SRC_TEXT;

    int i=0;
    while (fileTypes[i].ext) {
        if (0==stricmp(fileTypes[i].ext, ext)) {
            m_srcType = fileTypes[i].type;
            break;
        }
        i++;
    }
...
void CCrystalTextBuffer::EndUndoGroup()
{
    m_bUndoGroupKill = TRUE;
}
...
void CCrystalTextBuffer::BeginUndoGroup(BOOL bMergeWithPrevious /*= FALSE*/)
{
    ASSERT(! m_bUndoGroup);
    m_bUndoGroup = TRUE;
    m_bUndoBeginGroup = m_bUndoGroupKill || (m_nUndoPosition == 0 || ! bMergeWithPrevious);
    m_bUndoGroupKill = FALSE;
}
