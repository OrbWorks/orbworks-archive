...
BEGIN_MESSAGE_MAP(CCrystalTextView, CView)
    ...
    ON_COMMAND(ID_EDIT_FIND_CURRENT, OnFindCurrent)
...
{
    if (nColorIndex == COLORINDEX_CONSTANT) return TRUE;
    else return FALSE;
}
...
{
#ifdef SRCED
    switch (m_pTextBuffer->m_srcType) {
    case SRC_ASP:
        return ParseLineAsp(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_BASIC:
        return ParseLineBasic(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_BATCH:
        return ParseLineBatch(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_CPP:
        return ParseLineC(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_CSHARP:
        return ParseLineCSharp(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_ORBFORMS:
        return ParseLineOrbForms(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_HTML:
        return ParseLineHtml(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_JAVA:
        return ParseLineJava(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_PASCAL:
        return ParseLinePascal(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_PERL:
        return ParseLinePerl(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_PYTHON:
        return ParseLinePython(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_POWERSHELL:
        return ParseLinePowerShell(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_PHP:
        return ParseLinePhp(dwCookie, nLineIndex, pBuf, nActualItems);
    case SRC_RSRC:
        return ParseLineRsrc(dwCookie, nLineIndex, pBuf, nActualItems);
    default:
        return 0;
    }
#else
...
void CCrystalTextView::OnUpdateIndicatorSize(CCmdUI* pCmdUI)
{
    //ASSERT_VALIDTEXTPOS(m_ptCursorPos);
    CString stat;
    // @TODO: fill in actual byte size
    stat.Format(_T("%d bytes"), m_pTextBuffer->GetSize());
    pCmdUI->SetText(stat);
}
