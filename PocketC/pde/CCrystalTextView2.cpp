...
BOOL CCrystalTextView::OnMouseWheel(UINT nFlags, short zDelta, CPoint point)
{
    //CView::OnMouseWheel(nFlags, zDelta, point);
    int nPageLines = GetScreenLines();
    int nLineCount = GetLineCount();
    int nNewTopLine = m_nTopLine - zDelta * 3 / WHEEL_DELTA;
...
void CCrystalTextView::OnFindCurrent()
{
    m_pTextBuffer->EndUndoGroup();
    //AdjustTextPoint(point);
    ...
    if (ptStart.y == ptEnd.y) {
        ...
        LPCTSTR pszChars = GetLineChars(ptStart.y);
        int nChars = ptEnd.x - ptStart.x;

        if (nChars) {
            if (m_pszLastFindWhat)
                free(m_pszLastFindWhat);
            m_pszLastFindWhat = NULL;
            m_bLastSearch = true;

            m_pszLastFindWhat = (TCHAR*)malloc((nChars + 1) * sizeof(TCHAR));
            _tcsncpy(m_pszLastFindWhat, pszChars + ptStart.x, nChars);
            m_pszLastFindWhat[nChars] = 0;
