...
static ResizeData resizeData[] = {
    { MainForm, RESIZE_STRETCH, 4 },
    { MainProgsList, RESIZE_STRETCH },
    { MainCompileButton, RESIZE_Y_ATTACH_BOTTOM },
    { MainDebugCheckbox, RESIZE_Y_ATTACH_BOTTOM },
    { MainClassicCheckbox, RESIZE_X_ATTACH_RIGHT },
};
static int cResizeData = sizeof(resizeData) / sizeof(resizeData[0]);
...
static DIAtype diaType = DIA_TYPE_NONE;
...
void SetDIAAllowResize(bool allow) {
    switch (diaType) {
        ...
        case DIA_TYPE_PALM11:
        case DIA_TYPE_PALM10:
            PINSetInputTriggerState(allow ?
            	pinInputTriggerEnabled : pinInputTriggerDisabled);
...
bool HandleResizeNotification(UInt32 notificationType) {
    ...
    if (diaType != DIA_TYPE_PALM11) {
        ...
        WinGetDisplayExtent(&e.data.winDisplayChanged.newBounds.extent.x,
        	&e.data.winDisplayChanged.newBounds.extent.y);
...
bool HandleResizeEvent(word formID, EventPtr pEvent) {
    if (pEvent->eType == winDisplayChangedEvent) {
        FormPtr frmP = FrmGetFormPtr(formID);
        if (LayoutForm(formID)) {
            FrmDrawForm(frmP);
            return true;
        }
        return false;
    } else if (pEvent->eType == frmOpenEvent) {
        InitFormDIA(formID);
        LayoutForm(formID);
        return false; // each form must still process this event
    }
    return false;
}
