...
base_dbvector::base_dbvector(long delta) {
    nDelta = delta;
    nAlloc = nItems = 0;
    hMem = NULL;
    pt = NULL;
...
base_dbvector::~base_dbvector() {
}

void base_dbvector::init(DmOpenRef dbRef, long size, long type_size) {
    assert(hMem==NULL);
    this->dbRef = dbRef;
    if (size == 0) size = 1;
    recId = -1;
    hMem = (MemHandle)DmNewRecord(dbRef, &recId, size * type_size);
    if (!hMem) oom();
    pt = (void*)MemHandleLock(hMem);
    nAlloc = size;
}

void base_dbvector::clear() {
    nItems = 0;
}

long base_dbvector::add(void* pItem, long type_size) {
    if (nItems >= nAlloc) {
        MemHandleUnlock(hMem);
        MemHandle hMemNew = DmResizeRecord(dbRef, recId, type_size * (nItems + nDelta));
        if (!hMemNew) {
            // return to consistent state
            pt = (void*) MemHandleLock(hMem);
            oom();
    ...
    DmWrite(pt, type_size * nItems, pItem, type_size);
    return ++nItems;
}

void base_dbvector::reserve(long size, long type_size) {
    if (nItems + size > nAlloc) {
        long thisDelta = nItems + size - nAlloc;
        if (thisDelta % nDelta)
            thisDelta += nDelta - (thisDelta % nDelta);
        MemHandleUnlock(hMem);
        MemHandle hMemNew = DmResizeRecord(dbRef, recId, type_size * (nAlloc + thisDelta));
        if (!hMemNew) {
            // return to consistent state
            pt = (void*)MemHandleLock(hMem);
            oom();
